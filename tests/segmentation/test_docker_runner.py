...
def test_image_already_present_skips_pull(tmp_path: Path):
    ...
    with patch("menflow.segmentation.docker_runner.subprocess.run") as mock_run:
...
def test_image_missing_triggers_pull(tmp_path: Path):
    ...
    with patch("menflow.segmentation.docker_runner.subprocess.run", side_effect=_side):
        runner.ensure_image()
