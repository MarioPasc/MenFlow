...
2. Calls :class:`menflow.segmentation.docker_runner.BratsDockerRunner` against
   a single-subject staging dir.
3. Reduces the raw multi-class prediction to a binary whole-tumor mask via
   :func:`menflow.segmentation.output.wt_mask_from_prediction` and saves it
...
from menflow.segmentation.models import BRATS_MODELS, get_model
...
class SegmentBratsRoutineConfig:
    ...
    output_dir
        ...
        ``manifest.json``.
    model
        Model id (key of :data:`menflow.segmentation.models.BRATS_MODELS`).
    input_dir
